...
@dataclass
class JigParameters:
    jig_translate_y: float
    slicing_z: float
    x_wall: float
    y_wall: float
    z_wall: float
    slicing_z: float
    pre_knife_space: float
    post_knife_space: float
...
def prep_mold(nifti_path, label, iterations, reduction, scale, organ):
    ...
    if organ == 'brain':
        ...
        rotator = new_rotator(smoother.GetOutputPort())
        scaler = new_scaler(rotator.GetOutputPort(), scale)
    else:
...
def validate_profile(profile: dict[str, Any]) -> None:
    required_keys = [
        "organ", "jig_translate_y", "x_wall", "y_wall", "z_wall",
        "slicing_z", "pre_knife_space", "post_knife_space", "label",
        "iterations", "reduction", "scale", "tumor_laterality"
    ]
    missing_keys = [key for key in required_keys if key not in profile]
    if missing_keys:
        raise ValueError(f"Profile is missing required keys: {', '.join(missing_keys)}")
...
def unpack_profile(profile: dict[str, Any]) -> tuple:
    organ = profile["organ"]
    ...
    return (
        organ, jig_translate_y, x_wall, y_wall, z_wall, slicing_z,
        pre_knife_space, post_knife_space, label, iterations, reduction, 
        scale, laterality
        )

    # return (
    #     organ, jig_params, label, iterations,
    #     reduction,  scale, laterality
    #     )
...
def main(nifti_path: str, mold_stl_path: str, jig_stl_path: str, profile) -> None:
    ...
    #Setting the parameters from the profile
    organ, jig_translate_y, x_wall, y_wall, z_wall, slicing_z, pre_knife_space, post_knife_space, label, iterations, reduction, scale, laterality = unpack_profile(profile)

    # organ, jig_params, label, iterations, reduction, scale, laterality = unpack_profile(profile)
    #x_wall = jig_params.x_wall
    #y_wall = jig_params.y_wall
    #z_wall = jig_params.z_wall

    #Finding the slice thickness from the nifti header
    slice_thickness = find_slice_thickness(nifti)
    if organ =='brain' and slice_thickness < 5:
        slice_thickness *= 2
  
    #Creating the VTK mold polydata and bounds using the nifti mask and parameters from the profile
    mold_poly, mold_bounds = prep_mold(nifti_path, label, iterations, reduction, scale, organ)

    #Converting the mold polydata to an STL file
    mold_stl = make_mold_stl(mold_poly, mold_stl_path)

    #Creating the jig cq.Workplane object based on the mold bounds and jig parameters (from profile)
    ...
    #Creating the slicer cq.Workplane object based on the jig size and parameters
    slicer = prep_slicer(jig_size, slice_thickness, jig_translate_y, slicing_z, x_wall, y_wall)

    #Assembling the jig by slicing the jig with the slicer and cutting the mold from the sliced jig
    final_jig = assemble_jig(jig, slicer, slice_thickness, jig_size, z_wall, jig_translate_y, mold_stl, laterality)

    #Exporting the final jig to an STL file
...
if __name__ == "__main__":
    ...
    nifti = args.nifti
    mold_stl_path = args.mold_stl_path #need to flush out
    jig_stl_path = args.jig_stl_path
    ...
    #/Volumes/Siren/Brain_data/1.PatientDirectory/257/MRI/Processed/20241121_Mr_Brain_Rcbv_WO+W_Cont/brainmask_257.nii.gz
    #/Volumes/Siren/Brain_data/1.PatientDirectory/258/MRI/Processed/20250131_Mr_Brain_Rcbv_WO+W_Cont/brainmask.nii.gz
    #252 L 3.5
    # #254 R 4
    # GR 256 R 4
    # SE 257 L 3.99
    ...
    main(nifti, mold_stl_path, jig_stl_path, profile)
    



    # TODO:
    # DONE-ish - flush out profile/add profile file support
    # DONE - add tumor laterality 
    # - finish slice by slice logic
    # - flush out args | file profiles, default profile, 
    # - ???
    # - lint
    # - finish docstrings
